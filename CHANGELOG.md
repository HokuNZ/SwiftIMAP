...
- **Breaking.** `MimePart.body` (#38). It was the only public API exposing a MimeParser dependency type (`MimeBody`), so fully using it required `import MimeParser`. The decoded accessors (`decodedText`, `decodedData` — now a stored property) cover its uses without the dependency leak.
