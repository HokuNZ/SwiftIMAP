...
- **Breaking.** `listMessages(in:searchCriteria:charset:)` (deprecated since v1.1 in favour of `listMessageUIDs`) and `fetchMessageBySequence(sequenceNumber:in:items:)` (#39). Both are sequence-number based, so their results shift when the mailbox changes concurrently; the UID-based equivalents (`listMessageUIDs`, `fetchMessage(uid:)`) are stable. The README/Examples previously fed `listMessages` results into `fetchMessage(uid:)` — a latent bug this removal also fixes in the docs.
- **Breaking.** `searchMessagesComplex(in:from:to:subject:text:since:before:flags:excludeFlags:limit:)` (#39): a 9-parameter wrapper reimplementing `.and([...])`. Use `searchMessages(in:matching:)` with `SearchCriteria` values instead; the simple convenience wrappers (`searchMessagesFrom`, `searchUnreadMessages`, etc.) remain.
