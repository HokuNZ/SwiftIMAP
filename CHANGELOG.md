...
- `connect()` is now idempotent (#37): a call on an already-connected, healthy client is a no-op; a call on a disconnected or stale client (e.g. after the connection dropped) reconnects and re-authenticates; and concurrent calls coalesce onto a single attempt. Previously a second `connect()` threw `invalidState("Already connected or connecting")`, forcing consumers (e.g. MailTriage) to discard and rebuild the entire client after any error.
