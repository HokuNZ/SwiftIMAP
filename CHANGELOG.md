...
- `MessageSummary.referenceIDs: [String]` (#48): the `references` header parsed into bare message-IDs (angle brackets stripped, space- or comma-separated), for threading. The raw `references` string is retained. Callers that hand-parsed `references` can drop that code.
- `expectedUIDValidity: UInt32?` parameter (default `nil`) on `storeFlags` (all overloads), `moveMessage(s)`, `copyMessage(s)`, and `expunge(uids:in:)` (#48). When set, the operation is refused with the new `IMAPError.uidValidityChanged(expected:actual:)` if the mailbox's current `UIDVALIDITY` differs, guarding writes against UIDs from a stale mailbox view. The check rides on the `SELECT` the operation already issues, so it is atomic with the write and costs no extra round trip — replacing any consumer-side `STATUS`-then-write guard (which has a TOCTOU window). Default `nil` preserves existing behaviour exactly.
