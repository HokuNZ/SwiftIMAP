...
- Raw-RFC822 parsing for consumers with message bytes rather than a live IMAP `FETCH` (`.eml` importers, Maildir readers, webhook payloads, offline fixtures) (#61): `Envelope(parsingHeaders: [String: String])` builds a typed `Envelope` from a header dict (case-insensitive names; parses `From`/`Sender`/`Reply-To`/`To`/`Cc`/`Bcc`/`Subject`/`Date`/`Message-ID`/`In-Reply-To` into addresses, an RFC 2047 decoded subject, and a `Date`), and `MessageSummary.parse(rfc822: Data)` builds a full `MessageSummary` from message bytes (headers → `Envelope` → `references` populated; `uid`/`sequenceNumber` synthesised as `0`, `internalDate` from the `Date` header or now). Header parsing is independent of the MIME body (#67), so a body the MIME parser cannot handle does not lose the recoverable envelope; bytes are decoded as UTF-8 with an ISO-8859-1 fallback, and a leading mbox `From ` envelope line is stripped. It throws only when no RFC 822 headers can be found at all. The Date parser covers the common RFC 2822 forms: with and without leading weekday, 24-hour and AM/PM clocks, and a trailing `(...)` timezone comment.
...
- Documented that an `IMAPClient` wraps a single connection / selected mailbox: its mailbox-scoped operations are not safe to run concurrently on one instance (a `SELECT` from another in-flight operation can change the selected mailbox mid-sequence). Issue them serially or use one client per concurrent context. Per-client serialisation to enforce this is planned (#71). Also documented that a non-MOVE `moveMessage(s)` leaves the source flagged `\Deleted` until expunged, not fully removed (#74).
...
- `fetchMessage` and `searchMessages` now auto-add the attributes a `MessageSummary` requires — `UID`, `INTERNALDATE`, `RFC822.SIZE` — to the fetch when a caller passes a reduced `items` set (#72), extending the existing `UID` auto-injection. A reduced item set previously failed with `parsingError("Missing required message attributes")`; it now returns a complete summary. Non-breaking; the wire fetch carries a few more attributes when the caller omitted them.
...
- `parseMIMEContent(from:)` now decodes message bytes as UTF-8 with an ISO-8859-1 fallback instead of throwing `parsingError` on non-UTF-8 input (#70). Real mail frequently carries non-UTF-8 bytes in the body; Latin-1 maps every byte, so the body parses rather than failing at the decode step. It still throws if the MIME structure itself cannot be parsed.
