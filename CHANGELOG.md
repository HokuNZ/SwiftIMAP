...
- `fetchMessageBody` now reconnects and retries on a lost connection like `fetchMessage` (#46). A body fetch is an idempotent read, and the `peek` default means a retry does not double-clear `\Seen`; previously a mid-fetch drop failed the call outright with no reconnect attempt.
