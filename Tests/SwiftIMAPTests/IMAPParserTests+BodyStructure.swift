...
extension IMAPParserTests {
    func testParseBodyFetchUsesBodyStructure() throws {
        let input = "* 1 FETCH (BODY (\"TEXT\" \"PLAIN\" (\"CHARSET\" \"UTF-8\") NIL NIL \"7BIT\" 12 1))\r\n"
        ...
        guard let bodyStructure = attributes.compactMap({
            ...
            return XCTFail("Expected BODY attribute to parse as body structure")
        }

        XCTAssertEqual(bodyStructure.type, "TEXT")
        XCTAssertEqual(bodyStructure.subtype, "PLAIN")
        XCTAssertEqual(bodyStructure.parameters?["charset"], "UTF-8")
        XCTAssertEqual(bodyStructure.size, 12)
        XCTAssertEqual(bodyStructure.lines, 1)
    }
